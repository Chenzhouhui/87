...
module prewitt_core #(
    ...
    // Prewitt uses no 2x weighting, so magnitudes are smaller
    // Gx = -top0 + top2 - mid0 + mid2 - bot0 + bot2  (range: -1020 to +1020, signed 11-bit)
    // Gy = -top0 - top1 - top2 + bot0 + bot1 + bot2  (range: -1020 to +1020, signed 11-bit)
    reg signed [10:0] gx;
    reg signed [10:0] gy;
    reg [10:0] abs_gx;
    reg [10:0] abs_gy;
        ...
        if (!rst_n) begin
            ...
            if (gray_valid) begin
                ...
                if ((gray_x >= 2) && (gray_y >= 2)) begin
                    ...
                    edge_data  <= (mag > 12'd255) ? 8'hff : mag[7:0];
                    edge_x     <= out_x;
                    edge_y     <= out_y;
                    edge_valid <= 1'b1;
                end
